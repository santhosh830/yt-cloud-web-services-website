...
const NavBar = () => {
  ...
  return (
    <>
     <div className='w-screen h-20 z-20 fixed bg-gray-900
     text-white'>
        <div className='px-3 flex items-center justify-between w-full h-full'>
            ...
            <ul className='hidden md:flex'>
                {links.map(({id, link}) =>(
                    <li key={id} className='capitalize p-4 cursor-pointer hover:text-lightColor duration-200'><Link to={link} smooth duration={500}>{link}</Link></li>
                ))}
            </ul>
            ...
            <div className={navigation ? 'md:hidden fixed left-0 top-0 w-full h-full bg-gray-900/70 backdrop-blur': ""}>
              <div className={navigation ? 'fixed left-0 top-0 w-4/5 h-full bg-gray-900 text-white p-10 ease-in duration-500' : 
                ...
                <div className='mt-24 flex flex-col h-fit gap-12'>
                  ...
                  <ul className='capitalize'>
                    {links.map(({id, link}) =>(
                        <li key={id} className='py-4 text-2xl tracking-wider cursor-pointer'><Link onClick={() => setNavigation(false)} to={link} smooth duration={500} offset={-75}>{link}</Link></li>
                    ))}
                  </ul>
